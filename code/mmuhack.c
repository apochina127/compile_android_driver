...
pte_t *page_from_virt(uintptr_t addr) {
    ...
    pr_info("[CBHD] page_from_virt called with addr: 0x%llx\n", (unsigned long long) addr);
    if (!init_mm_ptr) {
    ...
    pr_debug("[CBHD] page_from_virt succes, virt (0x%llx), ptep @ %llx", (unsigned long long) addr,
...
int protect_rodata_memory(int mode, int nr) {
    ...
    if (mode == PRD_MDOE_V1) {
        ...
        if (result != 0) {
            printk(KERN_ERR "[CBHD] Failed to set memory to read-only mode\n");
            return -1;
        }
    }
    if (mode == PRD_MODE_V2) {
    ...
    if (mode == PRD_MODE_V3) {
        ptep = page_from_virt(addr);
        if (!pte_valid(READ_ONCE(*ptep))) {
            printk(KERN_INFO "[CBHD] failed to get ptep from 0x%lx\n", addr);
            return -2;
        }
        pte = READ_ONCE(*ptep);
...
int unprotect_rodata_memory(int mode, int nr) {
    ...
    if (mode == PRD_MDOE_V1) {
        ...
        if (area == NULL) {
            printk(KERN_ERR "[CBHD] Could not find vm area\n");
            return -1;
        }
        area->flags |= VM_ALLOC;

        result = my_set_memory_rw(addr, 1);
        if (result != 0) {
            printk(KERN_ERR "[CBHD] Failed to set memory to read/write mode\n");
            return -1;
        }
    }
    if (mode == PRD_MODE_V2) {
